...
//         "https://economic-backend-new.vercel.app/api/upload",
//         {
//           method: "POST",
//           headers: {
...
// app/components/FileList.tsx - Debug version
"use client";
...
const FileListComp: React.FC = () => {
    ...
    if (files) {
      files.forEach((file, index) => {
        if (file.type.startsWith("image/")) {
          ...
          testImg.onload = () =>
            console.log(`✅ Image ${index} loads: ${file.name}`);
          testImg.onerror = (e) =>
            console.error(`❌ Image ${index} fails: ${file.name}`, e);
        }
      });
    }
  }, [files]);
  ...
  return (
    <div>
      <h2>Debug: Uploaded Files ({files?.length || 0})</h2>
      ...
      {files?.map((file, index) => (
        ...
        >
          ...
          {file.type.startsWith("image/") ? (
            <div>
              <p>Testing image display:</p>
              <img
                src={file.url}
                alt={file.name}
                style={{ maxWidth: "200px", border: "2px solid green" }}
                onLoad={() => console.log(`✅ Rendered: ${file.name}`)}
                onError={() => console.log(`❌ Render failed: ${file.name}`)}
              />
            </div>
          ) : (
            <p>Not an image - showing as link</p>
          )}
